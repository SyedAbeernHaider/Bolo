    ...
    const timer = setInterval(() => {
      setProgress((prev) => {
        ...
        if (prev >= 100) {
          ...
          // 2. Trigger navigation (Side Effect)
          // We don't return the cleanup function to 'setProgress',
          // we just execute the logic.
          setTimeout(() => navigate("/name-input"), 500);

          // 3. Return the final number value
          return 100;
        }
        // 4. Increment
        return prev + 100 / 60;
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-500 to-teal-500 flex flex-col overflow-hidden relative border-8 border-gray-800">
          ...
          <div className="flex justify-center gap-4 mt-8">
            {[0, 1, 2].map((i) => (
              <motion.div
                ...
                style={{
                  backgroundColor: ["#fde047", "#ec4899", "#2dd4bf"][i],
                }}
                animate={{
                  y: [0, -15, 0],
                  scale: [1, 1.2, 1],
      ...
      <div className="absolute bottom-4 left-0 right-0 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-center items-center text-sm text-white filter drop-shadow-md z-10">
        <div className="flex items-center gap-2">
          <FiVolumeX className="w-4 h-4" /> Sound Off | Hyper-Animated
          Loading...
        </div>
      </div>
    </div>
